package com.godox.common.kdtablayout.widget.indicator
...
import com.godox.common.kdtablayout.KDTabLayout
import com.godox.common.kdtablayout.dpToPx
...
open class KDRecIndicator(tabLayout: KDTabLayout) : KDTabIndicator(tabLayout) {
    ...
    override fun onTabScrolled(startItem: Int, endItem: Int, scrolledFraction: Float) {
        ...
        if (tabLayout.childCount == 0) {
            ...
            if (startItem >= tabLayout.childCount || startItem < 0) {
                throw IndexOutOfBoundsException("startItem必须是tabLayout.childCount的范围内，startItem:${startItem},childCount:${tabLayout.childCount}")
            }

            if (endItem >= tabLayout.childCount || endItem < 0) {
                throw IndexOutOfBoundsException("endItem必须是tabLayout.childCount的范围内,endItem:${startItem},childCount:${tabLayout.childCount}")
            }
