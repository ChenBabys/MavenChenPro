...
class KDViewPagerHelper {
    ...
    fun bindViewPager2(viewPager: ViewPager2) {
        viewPager.unregisterOnPageChangeCallback(mViewPager2Callback)
        viewPager.registerOnPageChangeCallback(mViewPager2Callback)
    }
    ...
    fun bindViewPager(viewPager: ViewPager) {
        viewPager.removeOnPageChangeListener(mViewPagerCallback)
        viewPager.addOnPageChangeListener(mViewPagerCallback)
    }

    private val mViewPagerCallback = object : ViewPager.OnPageChangeListener {
        override fun onPageScrollStateChanged(state: Int) {
            this@KDViewPagerHelper.onPageScrollStateChanged(state)
        }
